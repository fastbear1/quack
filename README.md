...
[![Status](https://img.shields.io/badge/beta-0.37.1-blue)](https://github.com/fastbear1/quack/releases)
...
### Example of gorm tables declarations
----------------
See [Sandbox](./sandbox/Menu.md) cases

----------------
### Restrictions

#### Use as more as possible gorm tags

#### Primary keys
Correct ways to declare id primary key
...
#### Timestamp fields
Correct ways to declare timestamp fields:
```go
    type Table struct {
        ...
        CreaetdAt time.Time
        UpdatedAt `gorm:"type:timestamp without time zone;default:now()`
        ...
    }
```

#### Array fields
Nested array ignored by default in postgres, so it's better to use single array (text[] or integer[])
Correct way to use declare array fields
```go
    type Table struct {
        ...
...
#### Json fields
Correct way to declare json or jsonb fields
```go
    type Table struct {
        ...
